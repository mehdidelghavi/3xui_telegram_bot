    ...
    try {
        ...
        return {
            ...
            data: data.map(u => ([
                ...
                u.type,
                u.isActive
                    ? `<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-500/15 text-green-400 border border-green-500/30">
                        فعال
                    </span>`
                    : `<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-red-500/15 text-red-400 border border-red-500/30">
                        غیرفعال
                    </span>`,
                `<div class="flex gap-2">

                    <a href="/paymentMethods/details/${u.id}"
...
exports.getPaymentMethodById = async (paymentMethodId) => {
    try {
        ...
        return {
            success: true,
            message: "روش پرداخت با موفقیت یافت شد",
...
exports.deleteDetailById = async (detailId) => {
    try {
        const delDetail = await prisma.PaymentMethodDetail.delete({
            where: {
                id: Number(detailId)
            }
        });
        return {
            success: true,
            message: "جزییات پرداخت با موفقیت حذف شد"
        }
    } catch (e) {
        return {
            success: false,
...
exports.createDetailCardToCard = async (paymentMethodId, data) => {
    try {
        const createDetail = await prisma.PaymentMethodDetail.create({
            data: {
        ...
        return {
            success: true,
            message: "اطلاعات کارت با موفقیت ثبت شد",
            obj: createDetail
        }
    } catch (e) {
        return {
